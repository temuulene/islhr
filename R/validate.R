...
.islh_check_flag <- function(x, arg, call = rlang::caller_env()) {
  if (!is.logical(x) || length(x) != 1L || is.na(x)) {
    .islh_abort("{.arg {arg}} must be a single TRUE or FALSE.", call = call)
  }
  x
}
...
.islh_check_dimension <- function(x, arg, maximum = 200, call = rlang::caller_env()) {
  if (!is.numeric(x) || length(x) != 1L || is.na(x) || !is.finite(x) ||
      x <= 0) {
    .islh_abort("{.arg {arg}} must be one positive, finite number of inches.", call = call)
  }
  if (x > maximum) {
    .islh_abort(c(
...
# too large to open.
.islh_check_dpi <- function(x, arg = "dpi", maximum = 2400, call = rlang::caller_env()) {
  if (!is.numeric(x) || length(x) != 1L || is.na(x) || !is.finite(x) ||
      x < 1) {
    .islh_abort(
      "{.arg {arg}} must be one positive, finite resolution in dots per inch.", call = call)
  }
  if (x > maximum) {
    .islh_abort(c(
...
.islh_check_fraction <- function(x, arg, call = rlang::caller_env()) {
  if (!is.numeric(x) || length(x) != 1L || is.na(x) || !is.finite(x) ||
      x <= 0 || x > 1) {
    .islh_abort(c(
...
.islh_check_size <- function(x, arg = "base_size", maximum = 100, call = rlang::caller_env()) {
  if (!is.numeric(x) || length(x) != 1L || is.na(x) || !is.finite(x) ||
      x <= 0) {
    .islh_abort(
      "{.arg {arg}} must be one positive, finite font size in points.", call = call)
  }
  if (x > maximum) {
    .islh_abort(c(
...
.islh_check_count <- function(x, arg, call = rlang::caller_env()) {
  if (!is.numeric(x) || length(x) != 1L || is.na(x) || !is.finite(x) ||
      x != round(x) || x < 1) {
    .islh_abort("{.arg {arg}} must be one positive whole number.", call = call)
  }
  as.integer(x)
}

# A position inside a panel, as fractions from the bottom left corner.
.islh_check_position <- function(x, arg, call = rlang::caller_env()) {
