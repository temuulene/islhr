...
#'   DOCX renders; outside a render it selects plot-only setup.
...
islh_install_deps <- function(
  ...
  check <- islh_check(format = format, quiet = TRUE)
  wanted <- unique(c(check$missing, check$outdated))

  if (length(wanted) == 0L) {
    if (!isTRUE(quiet)) {
      .islh_inform(c(
        "v" = paste0(
          "Nothing to install; ", toupper(check$format),
  ...
  if (!isTRUE(quiet)) {
    after <- islh_check(format = check$format, quiet = TRUE)
    if (after$ok) {
